<template>
    <div class="w-screen h-screen bg-cover !bg-black">
        <div class="mx-auto py-16 h-full max-w-4xl grid grid-cols-4 gap-x-8">
            <img class="col-span-full mx-auto w-40 h-40 z-10" :src="'/img/camps.png'">
            <div v-for="team in statuses.list" :key="team.id" class="text-gray-200 flex flex-col items-center gap-y-2 transition-all duration-[1500ms] mt-auto">
                <div class="relative w-full rounded transition-all duration-[1500ms] shrink-0 bg-gray-700 bg-opacity-40" :style="{ height: BASE_HEIGHT + 'px' }">
                    <div class="absolute bottom-0 w-full shadow-lg rounded transition-all duration-[1500ms] shrink-0"
                        ...
                        :style="{ height: (Number(team.camps)/5000) * BASE_HEIGHT + 'px' }">
                        <div class="absolute inset-x-0 -top-12 text-center text-3xl font-bold">{{ team.camps }}€</div>
                    </div>
                </div>
